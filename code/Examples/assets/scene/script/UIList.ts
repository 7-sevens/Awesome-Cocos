...
export default class UIList extends Component {
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            case SelectedType.SINGLE: {
                ...
                t._selectedId = val;
                if (item) {
                    listItem = item.getComponent(UIListItem);
                    listItem.selected = true;
                }
                if (t._lastSelectedId >= 0 && t._lastSelectedId != t._selectedId) {
                    let lastItem: any = t.getItemByListId(t._lastSelectedId);
                    if (lastItem) {
                        lastItem.getComponent(UIListItem).selected = false;
                    }
                }
                if (t.selectedEvent) {
    ...
    setTemplateItem(item: any) {
        ...
        let com: any = item.getComponent(UIListItem);
        let remove = false;
        if (!com)
            remove = true;
        // if (com) {
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            ...
            let listItem: UIListItem = item.getComponent(UIListItem);
    ...
    _updateListItem(listItem: UIListItem) {
        if (!listItem)
            return;
